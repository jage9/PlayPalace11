...
**Grid mode for board games — READ THIS:** If the game has a board or grid (chess board, game map, spatial layout), you **must** use the server's built-in grid mode. Override `rebuild_player_menu` in your game class and pass `grid_enabled=True` and `grid_width=N` to `show_menu`. Do not display a grid as a flat list — it will be unusable. However, grid mode must only be active when the grid items are actually in the menu. Check that the grid items are present before enabling it; if the menu is showing something else (lobby, promotion choices, non-spatial actions), set `grid_enabled=False`. See Chess for a reference implementation.
